    ...
    fn from(err: TypeError) -> Self {
        match err {
            ...
            TypeError::AnnotationConflict {
                ...
                span,
                format!("type annotation conflict: annotation says {annotation}, inferred {inferred}"),
            ),
            TypeError::NonExhaustiveMatch { span } => {
                diagnostics::CompilerError::type_error(span, "non-exhaustive match")
            }
            TypeError::FieldNotFound { field, span } => {
                diagnostics::CompilerError::type_error(span, format!("field `{field}` not found on record"))
            }
            TypeError::NumericOverflow { ty, span } => {
                diagnostics::CompilerError::type_error(span, format!("numeric literal overflows type `{ty}`"))
            }
        }
    }
}
