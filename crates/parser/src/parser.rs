...
struct Parser<'a> {
    ...
    /// Monotonically increasing counter — every AST node gets a unique [`NodeId`].
    next_node_id: u32,
}
...
impl<'a> Parser<'a> {
    ...
    /// Allocates the next [`NodeId`] and advances the counter.
    ...
    fn parse_primary(&mut self) -> Result<&'a Expr<'a>, ParseError> {
        ...
        let expr = match &peeked.kind {
            ...
            TokenKind::Ident(name) => {
                let tok = self.advance();
                let id = self.next_id();
                self.arena.alloc(Expr::Ident(id, name, tok.span))
            }
            TokenKind::Int(text) => {
                let tok = self.advance();
                let id = self.next_id();
                self.arena.alloc(Expr::IntLiteral(id, text, tok.span))
            }
            TokenKind::Float(text) => {
                let tok = self.advance();
                let id = self.next_id();
                self.arena.alloc(Expr::FloatLiteral(id, text, tok.span))
            }
            TokenKind::Str(text) => {
                let tok = self.advance();
                ...
                let id = self.next_id();
                self.arena.alloc(Expr::Str(id, arena_str, tok.span))
            }
            TokenKind::True => {
                let tok = self.advance();
                let id = self.next_id();
                self.arena.alloc(Expr::Bool(id, true, tok.span))
            }
            TokenKind::False => {
                let tok = self.advance();
                let id = self.next_id();
                self.arena.alloc(Expr::Bool(id, false, tok.span))
            }
        ...
        loop {
            if self.check(&TokenKind::OpenParen) {
                ...
                if self.match_token(&TokenKind::OpenParen) {
                    ...
                    let func = self.arena.alloc(Expr::Ident(func_id, field, field_span));
    ...
    fn parse_pattern_atom(&mut self) -> Result<&'a Pattern<'a>, ParseError> {
        ...
        match &peeked.kind {
            TokenKind::Underscore => {
                let tok = self.advance();
                let id = self.next_id();
                Ok(self.arena.alloc(Pattern::Wildcard(id, tok.span)))
            }
            TokenKind::Int(val) => {
                let tok = self.advance();
                let id = self.next_id();
                Ok(self
                    .arena
                    .alloc(Pattern::Literal(id, LiteralPattern::Int(val), tok.span)))
            }
            TokenKind::Float(val) => {
                let tok = self.advance();
                let id = self.next_id();
                Ok(self
                    .arena
                    .alloc(Pattern::Literal(id, LiteralPattern::Float(val), tok.span)))
            }
            TokenKind::Str(val) => {
                let tok = self.advance();
                let id = self.next_id();
                Ok(self
                    .arena
                    .alloc(Pattern::Literal(id, LiteralPattern::Str(val), tok.span)))
            }
            TokenKind::Backtick => {
                let start_tok = self.advance();
            ...
            TokenKind::True => {
                ...
                Ok(self
                    .arena
                    .alloc(Pattern::Literal(id, LiteralPattern::Bool(true), tok.span)))
            }
            TokenKind::False => {
                let tok = self.advance();
                let id = self.next_id();
                Ok(self
                    .arena
                    .alloc(Pattern::Literal(id, LiteralPattern::Bool(false), tok.span)))
            }
            TokenKind::Ident(name) => {
                let tok = self.advance();
                if name
                    ...
                    .is_some_and(|c: char| c.is_ascii_uppercase())
                {
                    ...
                    let id = self.next_id();
                    Ok(self.arena.alloc(Pattern::Binding(id, name, tok.span)))
                }
            }
            TokenKind::OpenBrace => {
                let start_tok = self.advance();
...
mod tests {
    ...
    fn test_parse_expr_atom() {
        ...
        assert!(matches!(
            ...
            Expr::IntLiteral(_, "42", _)
        ));
        assert!(matches!(
            parse_expr_helper("3.14", &bump),
            Expr::FloatLiteral(_, "3.14", _)
        ));
        assert!(matches!(
    ...
    fn test_parse_application_and_desugaring() {
        ...
        match expr {
            Expr::Application { func, args, .. } => {
                assert!(matches!(&**func, Expr::Ident(_, "f", _)));
        ...
        match desugared {
            Expr::Application { func, args, .. } => {
                assert!(matches!(&**func, Expr::Ident(_, "map", _)));
                assert_eq!(args.len(), 2);
                assert!(matches!(args[0], Expr::Ident(_, "x", _)));
                assert!(matches!(args[1], Expr::Ident(_, "f", _)));
            }
    ...
    fn test_parse_binary_precedence() {
        ...
        match expr {
            Expr::Binary {
                ...
                assert!(matches!(&**left, Expr::Ident(_, "a", _)));
                match &**right {
    ...
    fn test_parse_if_expr() {
        ...
        match expr {
            Expr::If {
                ...
                assert!(matches!(&**condition, Expr::Bool(_, true, _)));
    ...
    fn test_parse_match_expr() {
        ...
        match expr {
            Expr::Match { subject, arms, .. } => {
                assert!(matches!(&**subject, Expr::Ident(_, "x", _)));
    ...
    fn test_parse_field_access_and_index() {
        ...
        match idx_expr {
            Expr::Index { array, index, .. } => {
                ...
                assert!(matches!(&**index, Expr::Ident(_, "c", _)));
            }
    ...
    fn test_node_ids_are_unique() {
        let bump = Bump::new();
        // Parse two integer literals — they must get distinct NodeIds.
        let program = parse("let a = 1\nlet b = 2", &bump).unwrap();
        let ids: std::collections::HashSet<u32> = program.decls.iter().map(|d| d.id().0).collect();
        assert_eq!(
            ids.len(),
            program.decls.len(),
            "decl NodeIds must be unique"
        );
    }
}
